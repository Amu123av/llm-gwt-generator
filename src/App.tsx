...
export default function App() {
  const [activeTab, setActiveTab] = useState<'generator' | 'comparison' | 'inspector' | 'benchmark' | 'batch'>('generator');
  ...
  const handleGenerate = async (overrideInput?: RequirementInput) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || 'Failed to generate test cases.');
      }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Tab 3: Prompt Kit Inspector */}
        {activeTab === 'inspector' && <PromptKitInspector />}

        {/* Tab 4: Benchmark Dataset Library */}
        {activeTab === 'benchmark' && (
          <BenchmarkLibrary onSelectAndRun={handleSelectFromBenchmark} />
        )}

        {/* Tab 5: Batch Run Evaluation */}
        {activeTab === 'batch' && <BatchSimulator />}
      </main>

      {/* Footer */}
      <footer className="border-t border-slate-900 bg-slate-950 py-4 text-center text-xs text-slate-500">
        <p>
          LLM-Assisted Requirement-to-Test-Case Generation Framework • Google AI Studio Prompt Kit
        </p>
      </footer>
    </div>
  );
